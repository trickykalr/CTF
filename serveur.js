...
// CORRIGÉ : ajout de _insuranceTimer: null
function createRoom(code) {
  return {
    ...
    _betTimer: null,
    _insuranceTimer: null,
    gameOptions: { ...DEFAULT_OPTIONS },
  };
}

// ── Broadcast ─────────────────────────────────────────────────
// CORRIGÉ : handTotal uniquement sur les cartes visibles
  ...
  const publicState = {
    ...
    players: room.players.map((p, i) => {
      ...
      return {
        ...p,
        hand,
        // CORRIGÉ : filtre les cartes cachées avant calcul
        handTotal:      handTotal(hand.filter(c => !c.hidden)),
        splitHandTotal: p.splitHand?.length ? handTotal(p.splitHand.filter(c => !c.hidden)) : 0,
      };
    }),
  };
...
function startInsurancePhase(room) {
  ...
  room.players.forEach(p => {
    if (p.isBot && p.role === 'player') p.insuranceDeclined = true;
  });
...
function checkInsuranceComplete(room) {
  ...
  room.players.forEach(p => {
    ...
    if (dealerBJ) {
      ...
      p.balance += p.insuranceBet * 2 + p.insuranceBet;
...
function dealCardsForRoom(room) {
  ...
  room.players.forEach(p => { if (p.bet > 0) p.balance -= p.bet; });
  room.players.forEach(p => {
  ...
  const dealer      = room.players[room.dealerIdx];
  const visibleCard = dealer.hand[0];
...
// CORRIGÉ : dealerDelta tient compte des paiements d'assurance
function computeResultsWithSplit(room) {
  ...
  const updated = room.players.map(p => {
    ...
    // Tenir compte des paiements d'assurance déjà appliqués
    if (p.insuranceResult === 'win') dealerDelta -= p.insuranceBet * 2;
...
// ── handleLeave — logique commune déconnexion / quitter ───────
// CORRIGÉ : code dupliqué extrait en une seule fonction
function handleLeave(socket) {
  ...
  if (room.players.length === 0 || room.players.every(p => p.isBot)) {
    if (room._betTimer)       clearTimeout(room._betTimer);
    if (room._insuranceTimer) clearTimeout(room._insuranceTimer);
    delete rooms[code];
  } else {
    room.dealerIdx = room.players.findIndex(p => p.role === 'dealer');
    if (leaving) io.to(code).emit('toast', `${leaving.name} a quitté`);
    broadcastState(code);
  }
}
...
io.on('connection', (socket) => {
  ...
  socket.on('setOptions', (opts, cb) => {
    ...
    room.gameOptions = {
      numDecks:    Math.min(Math.max(parseInt(opts.numDecks)    || DEFAULT_OPTIONS.numDecks,    1), 8),
      bjPayout:    [1, 1.2, 1.5].includes(parseFloat(opts.bjPayout)) ? parseFloat(opts.bjPayout) : 1.5,
      betTimerSec: Math.min(Math.max(parseInt(opts.betTimerSec) || DEFAULT_OPTIONS.betTimerSec, 10), 120),
      maxBet:      Math.min(Math.max(parseInt(opts.maxBet)      || DEFAULT_OPTIONS.maxBet,      10), 9999),
      maxSplits:   Math.min(Math.max(parseInt(opts.maxSplits)   || DEFAULT_OPTIONS.maxSplits,    1), 3),
    };
    cb?.({ ok: true });
    broadcastState(room.code);
  });
  ...
  socket.on('hit', (_, cb) => {
    const room = getRoom(socket);
    if (!room || room.phase !== 'playing') return cb?.({ ok: false, error: "Ce n'est pas la phase de jeu." });
    const p   = getPlayer(socket, room);
    if (!p || p.isBot)                     return cb?.({ ok: false, error: 'Action non autorisée.' });
    if (room.players.indexOf(p) !== room.currentPlayerIdx) return cb?.({ ok: false, error: 'Pas ton tour.' });
  ...
  socket.on('dealerReveal', (_, cb) => {
    const room = getRoom(socket);
    if (!room || room.phase !== 'dealer')  return cb?.({ ok: false, error: "Ce n'est pas le tour du croupier." });
  ...
  // CORRIGÉ : leaveRoom et disconnect utilisent handleLeave
